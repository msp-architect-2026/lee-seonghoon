...
| **MetalLB** | 온프레미스 K8s 환경에서도 클라우드와 동일하게 외부 사용자가 서비스에 접근할 수 있는 실제 IP 주소를 제공합니다. | L2/BGP 로드밸런서: 클라우드 종속 없이 베어메탈 클러스터에 `LoadBalancer` 타입 서비스를 구현하여, Nginx Ingress에 안정적인 External IP를 할당하고 외부 트래픽 진입점을 확보합니다. |
| **Flannel** | K8s 클러스터 내 여러 노드에 분산된 FastAPI, AI 분석 파드들이 IP 충돌 없이 서로 안정적으로 통신할 수 있게 합니다. | CNI 오버레이 네트워크: VXLAN 기반 가상 네트워크를 구성하여 노드 간 파드-투-파드 통신을 투명하게 라우팅하며, 경량 구조로 AI 추론 트래픽의 내부 지연을 최소화합니다. |
