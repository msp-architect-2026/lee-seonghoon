...
스마트폰으로 접속하면 로그인 화면이 나와요. 소셜 로그인으로 간단히 시작해요.

---

**2. 카메라 화면**

로그인하면 바로 카메라가 켜져요. 화면 중앙에 얼굴을 맞출 수 있는 가이드라인이 뜨고, 조명이 어두우면 촬영 버튼이 비활성화되면서 "밝은 곳으로 이동해 주세요"라고 알려줘요. 조명이 괜찮아지면 버튼이 활성화되고 찍을 수 있어요.

---

**3. 촬영 → 서버로 전송**

찍는 순간 이미지가 서버로 전송돼요. 서버는 "접수했어요" 하고 바로 응답을 보내고, 실제 분석은 뒤에서 따로 돌려요. 유저를 기다리게 하지 않으려고요.

---

**4. 분석 중 화면**

단순한 로딩 스피너 대신, 찍은 얼굴 사진 위로 레이저 스캔 애니메이션이 쭉 내려가면서 "피부톤 분석 중...", "컬러 매칭 중..." 텍스트가 바뀌어요. 이 1~2초가 지루하지 않게 만든 장치예요.

뒤에서는 서버가 1초마다 "분석 끝났어?"라고 체크하고 있다가, 끝나면 자동으로 결과 화면으로 넘어가요.

---

**5. 결과 화면**

화면의 70%를 진단된 컬러 팔레트로 가득 채워요. "가을 웜톤이에요" 같은 문구와 함께 실제 어울리는 색상 5개가 큼직하게 보여요.

아래쪽을 스크롤하면 구체적인 추천이 나와요.
...
예전에 진단받은 결과들을 날짜별로 모아볼 수 있어요. "3개월 전이랑 지금이랑 결과가 다르네" 같은 걸 비교할 수 있어요.

---

### 한 줄 요약

> 찍는다 → 레이저 스캔 애니메이션 잠깐 → 내 얼굴에 맞는 색깔 팔레트가 화면 가득 뜬다
>
